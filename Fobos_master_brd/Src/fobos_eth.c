...
static uint8_t homing_checking = 0;
...
#define CMD_HOMING
	  if(fobos_eth_buf->fobos_protocol_buf_t.bytes_in_packet_N == 0)
	  {
		...
		can_tx_func(&hfdcan2, 0x620+2, 2, can_data_buf);//C rack homing
	      if(homing_checking){
	...
	case FOBOS_CMD_ROTATION://21 rotation
	#define CMD_ROTATION
		  if(fobos_eth_buf->fobos_protocol_buf_t.bytes_in_packet_N == 1)
		  {
			...
			fobos_eth_buf->fobos_protocol_buf_t.data[0] = FOBOS_ETH_ERR_NO;
			fobos_eth_protocol_send(FOBOS_CMD_ROTATION, fobos_eth_buf->fobos_protocol_buf_t.bytes_in_packet_N, fobos_eth_buf);
		    }
		    else{
		      fobos_eth_buf->fobos_protocol_buf_t.data[0] = FOBOS_ETH_ERR_PA;
...
#define CMD_BASING_STATEMENT
	...
	case FOBOS_CMD_MOTION://24, линейное перемещение мотора
#define CMD_MOTION
	  /*if(xHoming == NULL)
	    xHoming = xTaskCreate(homing_process, "homing", 128, (void*)0, tskIDLE_PRIORITY, &xHoming_Handle);*/
	  {
		...
		else if(fobos_eth_buf->fobos_protocol_buf_t.data[0] == 1){ // calibration position
		    if(xPosition_func == NULL)
		    xPosition_func = xTaskCreate(position_mode_process_calibration, "MotorCALIB", 128,(void*)0, 0, &xPosition_Handle);
		    fobos_eth_buf->fobos_protocol_buf_t.data[0] = FOBOS_ETH_ERR_NO;
		}
		else
		  {
	    ...
	    fobos_eth_protocol_send(FOBOS_CMD_MOTION, fobos_eth_buf->fobos_protocol_buf_t.bytes_in_packet_N, fobos_eth_buf);
	  }
	  break;

	case FOBOS_CMD_MOTION_STATUS: //25 cmd
#define CMD_MOTION_STATUS
	  if(fobos_eth_buf->fobos_protocol_buf_t.bytes_in_packet_N == 0)
	  {
	  ...
	  fobos_eth_protocol_send(FOBOS_CMD_MOTION_STATUS, fobos_eth_buf->fobos_protocol_buf_t.bytes_in_packet_N, fobos_eth_buf);
	  break;
  ...
  uint8_t can_data_tx[4] = {0};//can_data_tx[0] младший байт
   ...
   while((canopen_rcv.values_t.data[0] & 0b01100111) != 0x23)
 	...
 	while((canopen_rcv.values_t.data[0] & 0b01100111) != 0x27)
   	...
   	can_data_tx[0] = 0x01;//CALIBRATION POSITION!!!! 90.000 мкм
   	canopen_req_resp_sdo(0x600+1, 0x2B,0x2080,0,can_data_tx, &canopen_rcv);
