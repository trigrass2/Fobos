 ...
 *                  	    			4 - не исп.(по умолч. 0)
 *                  	    			5 - не исп.(по умолч. 0)
 *                      			6 - состояние "датчик поворота нижний аварийный"	(0 - сработал концевик)	(S1)
 *                      			7 - состояние "датчик поворота верхний аварийный"	(0 - сработал концевик)	(S2)
 *				DATA2 -	Bit's number:
 *						0 - не исп.(по умолч. 0)
 *						1 - состояние "аварийный выключатель линейного мотора"	1 - сработал выключатель, 0 - не сработал
 *						2 - состояние "аварийный выключатель линейного мотора" 	1 - сработал выключатель, 0 - не сработал
 *						3 - состояние "датчик положения стола ЛЕВЫЙ"		1 - сработал, 0 - не сработал
 *						4 - состояние "датчик положения стола ПРАВЫЙ"		1 - сработал, 0 - не сработал
 *						5 - не исп.(по умолч. 0)
 *						6 - не исп.(по умолч. 0)
 *						7 - не исп.(по умолч. 0)
 ...
 *				DATA1 != 0 - мотор готов для работы, DATA1 = 0 - мотор не готов
 *
 * 	Версия прошивки
 *	===TX
 *	CMD = 14	N = 0
 ...
 *
 * 	Сбазировать (HOMING) линейный мотор
 *	===TX
 *	CMD = 20	N = 0
 *	===RX
 ...
 *
 *	Перемещение линейного мотора
 *	===TX
 *	CMD = 24	N = 1	DATA0 - 2 - переместить линейный мотор влево, 3 - вправо (относ. лицевой стороны фобоса)
 *	===RX
 *	CMD = 24	N = 1	DATA0 = 0..6	// Статус ответа. Значение из диапазона FOBOS_ETH_ERR_NO...FOBOS_ETH_ERR_CMD
 *
 *	Проверка достижения ранее заданной точки
 *	===TX
 *	CMD = 25	N = 0
 *	===RX
...
#define FOBOS_CMD_BASING			20	// выполнить базирование (HOMING)
#define FOBOS_CMD_WORK				21	// команды работы с фобосом
#define FOBOS_CMD_LAMP				22	// управление лаймпой "Не входить"
#define FOBOS_CMD_BASING_STATEMENT		23	// проверка базирования С-рамы
#define FOBOS_CMD_START				24	//старт передвижения линейного мотора
#define FOBOS_CMD_START_STATUS			25	//Отслеживание завершения перемещения линейного мотора и С-рамы
